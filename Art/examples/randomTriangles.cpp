...
int main()
{
    ...
    std::mt19937 mt(rd());
    std::uniform_int_distribution<std::int16_t> random_distribution_position(-256, 256);
    ...
    //imagem.drawTriangle(5, 100, 200, 300, 400, 500, corPreto);
    {
        ...
        for (std::uint32_t i = 0; i < 256; ++i)
        {
            ...
            const std::int16_t randomPosition3X = random_distribution_position(mt);
            const std::int16_t randomPosition3Y = random_distribution_position(mt);

            imagem.drawTriangle(randomPosition1X, randomPosition1Y, randomPosition2X, randomPosition2Y, randomPosition3X, randomPosition3Y, corPreto);
        }
    }

    imagem.writeImagePPM("../images_output/randomTriangles.ppm");
    imagem.writeImageBMP("../images_output/randomTriangles.bmp");
    return 0;
}
